...
if play_on then
   ...
   state_msg="<b><font color=\"green\">" .. translate("运行中") .. "</font></b>"  .. "<p><p>Current Server Address    " .. server_info 

else                                                                                                                  
     
   state_msg="<b><font color=\"red\">" .. translate("未启动")  .. "</font></b>"
                                                                                                                 
end  


m = Map("switchlanplay", translate("Switch Lan Play"), translate("与好友一起联机狂欢吧！！！") .. "<p><p>" ..translate("Status") .. " - " .. state_msg)
...
f = m:section(Table,inits,translate("服务器列表"),translate("清单"))

s_name = f:option(DummyValue, "s_name", translate("服务器名称"))

s_address = f:option(DummyValue, "s_address",translate("服务器地址")) 
s_description = f:option(DummyValue, "s_description", translate("服务器介绍"))

--function s_name.cfgvalue(self, section)
--end


start = f:option(Button, "start", translate("运行"))                                                 
...
uplist = r:option(Button, "uplist", translate("下载服务器列表"), translate("After pressing the button please wait 10 seconds<br><font color='blue'>Old file in /etc/switchlanplay_list.json-bak</font>"))
...
function uplist.write(self, section, value)                                                                                   
        ...
        sys.call("wget -O /etc/switchlanplay_list.json --no-check-certificate 'https://kenneth06.coding.net/p/switch-lan-play/d/lanplay-server-openwrt/git/raw/master/switchlanplay_list.json'")              
        os.execute("sleep " .. 9 )
	luci.http.write("<script>location.href='./switchlanplay';</script>")                                                                                                                                                                          
        return                                                                                                               
end 
